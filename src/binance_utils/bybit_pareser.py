...
def parse_binance_messages(messages: List[Dict], output_file: str = 'bybit_futures.json'):
    """
    ...
    for msg in messages:
        ...
        print(tokens)
        if 'bybit' in tokens and 'futures' in tokens:
            results.append({
                'timestamp': str(msg.get('date', '')),
                'message': text
            })
    
    with open(RESULT_DIR + '/' + output_file, 'w', encoding='utf-8') as f:
